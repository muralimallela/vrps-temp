...
export const PhotoGalleriesClient: FC<{ isTelugu?: boolean }> = ({ isTelugu = false }) => {
        ...
        <header className="mb-10 rounded-3xl border border-[#e4c69d] bg-white/85 p-6 md:p-10 shadow-[0_20px_40px_-24px_rgba(90,28,22,0.45)] backdrop-blur">
          <div className="flex items-center gap-2 mb-3">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-[#0F5F54] px-3.5 py-1 text-xs font-semibold uppercase tracking-wider text-white shadow-sm">
              ...
              {isTelugu ? "VRPS చిత్రమాలిక" : "VRPS Photo Archives"}
            </span>
          </div>
          <h1 className="text-3xl font-black leading-tight text-[#3d120d] md:text-4xl lg:text-5xl">
            {isTelugu ? "సమాజ చిత్రమాలిక & మహాసభలు" : "Community Photo Galleries"}
          </h1>
          <p className="mt-3 max-w-3xl text-sm leading-relaxed text-[#5a3a2e] md:text-base">
            {isTelugu
              ? "మహాసభలు, సంక్షేమ కార్యక్రమాలు, యువజన సదస్సులు మరియు సాంస్కృతిక వేడుకల చిత్రమాలిక."
            ...
            {filteredItems.map((item, index) => (
              ...
              >
                ...
                <div className="relative h-60 w-full overflow-hidden rounded-xl bg-gray-100">
                  <Image
                    src={item.src}
                    alt={`${item.title} - VRPS Photo Archive`}
                    fill
            ...
            >
              ...
              <div className="relative w-full h-[65vh] md:h-[73vh] flex items-center justify-center rounded-2xl overflow-hidden bg-black/40 border border-white/10 shadow-2xl">
                <Image
                  ...
                  alt={`${selectedItem.title} - VRPS Photo Archive`}
                  fill
                  sizes="100vw"
