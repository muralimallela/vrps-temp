...
export default function CommunitySupportersClient({ isTelugu = false }: { isTelugu?: boolean }) {
      ...
      <section className="mx-auto max-w-6xl">
        ...
        <header className="mb-8 rounded-2xl border border-[#e4c69d] bg-white/85 p-6 shadow-[0_20px_40px_-24px_rgba(90,28,22,0.45)] backdrop-blur md:p-8">
          <p className="mb-2 inline-block rounded-full bg-[#6A160A] px-3 py-1 text-xs font-semibold uppercase tracking-wider text-white">
            {isTelugu ? "పారదర్శకత & సహాయకులు" : "Transparency & Supporters"}
          </p>
          <h1 className="text-3xl font-black leading-tight text-[#3d120d] md:text-4xl">
            {isTelugu ? "సమాజ సహాయకులు & దాతలు" : "Our Generous Supporters"}
          </h1>
          <p className="mt-3 max-w-3xl text-sm leading-relaxed text-[#5a3a2e] md:text-base">
            {isTelugu
              ...
              : "Heartfelt gratitude to the patrons and supporters driving our mission forward. Transparency is the core of our community trust."}
          </p>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 rounded-xl border border-[#eddcc8] bg-[#fffaf4] px-4 py-2 text-sm font-bold text-[#3d120d]">
              ...
              <span>
                <strong className="text-[#6A160A]">{pagination.total}</strong>{" "}
                {isTelugu ? "విరాళాలు నమోదయ్యాయి" : "Total Contributions"}
              </span>
            </div>
            <div className="flex items-center gap-2 rounded-xl border border-[#eddcc8] bg-[#fffaf4] px-4 py-2 text-sm font-semibold text-[#6a4a3b]">
              ...
              <span>{isTelugu ? "100% పారదర్శకత" : "100% Transparent Giving"}</span>
            </div>
          </div>
        </header>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-16 rounded-2xl border border-[#e4c69d] bg-white p-8 shadow-sm">
            ...
            <p className="text-sm font-bold text-[#6A160A]">Loading supporter records...</p>
          </div>
        ) : error ? (
          ...
          <div className="rounded-2xl border border-[#e4c69d] bg-white p-12 text-center shadow-sm">
            <h2 className="text-xl font-bold text-[#3d120d]">Be the First Supporter</h2>
            <p className="mt-2 text-sm text-[#6a4a3b]">
              No public contributions yet. Help kickstart our outreach initiatives today!
            </p>
          </div>
        ) : (
          <>
            ...
            {/* Pagination */}
            {pagination.pages > 1 && (
              <nav aria-label="Pagination Navigation" className="flex flex-col sm:flex-row justify-center items-center gap-3 pt-6 border-t border-[#e4c69d]">
                <button
