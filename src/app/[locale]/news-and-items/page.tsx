...
const NewsGallery: FC = () => {
    ...
    const fetchNews = async () => {
      try {
        ...
        if (body.success && Array.isArray(body.data) && body.data.length > 0) {
          const mapped: NewsItem[] = body.data.map((item: any) => ({
            id: item._id || item.$id,
            src: item.src,
            title: item.title,
  ...
  const shareWhatsApp = (item: NewsItem) => {
    const shareText = `*VRPS News Highlight*\n\n*${item.title}*\nCategory: ${item.category}\n\nCheck update here: ${window.location.href}`;
      ...
      <section className="mx-auto max-w-7xl">
        ...
        <div className="mb-10 rounded-3xl border border-[#e4c69d] bg-white/85 p-6 md:p-10 shadow-[0_20px_40px_-24px_rgba(90,28,22,0.45)] backdrop-blur">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-relaxed text-[#5a3a2e] md:text-base">
            Explore recent press highlights, community conventions, welfare drives, and developmental milestones across all regional committees.
          </p>
              ...
              >
                ...
                <div className="relative h-60 w-full overflow-hidden rounded-xl bg-gray-100">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 transition-opacity duration-300 group-hover:opacity-100 backdrop-blur-[2px]">
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-white/90 px-3.5 py-1.5 text-xs font-bold text-[#3d120d] shadow-lg">
        ...
        {/* Lightbox Modal with Mobile Touch Swipe & Action Controls */}
            ...
            >
              ...
              <div className="relative w-full mt-3 bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl p-4 text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-gray-300 mt-0.5">Category: {selectedItem.category} • {selectedItem.date}</p>
                </div>
                ...
                {/* Interactive Share Popover Menu */}
                {isShareMenuOpen && (
                  <div className="absolute right-4 bottom-16 z-50 w-64 rounded-2xl border border-white/20 bg-slate-900/95 p-3 shadow-2xl backdrop-blur-xl text-white animate-fadeIn">
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        <span className="flex items-center gap-2.5">
                          {copied ? <HiOutlineCheck className="h-4 w-4 text-emerald-400" /> : <HiOutlineClipboardDocument className="h-4 w-4" />}
                          {copied ? "Link Copied!" : "Copy Page Link"}
                        </span>
                        {copied && <span className="text-[10px] font-bold text-emerald-400">Done</span>}
                      </button>
                    </div>
                  </div>
                )}
