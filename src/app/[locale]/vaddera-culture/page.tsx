...
export async function generateMetadata({
  ...
  const data = isTelugu ? SEO_PAGE_DATA.vadderaCulture.te : SEO_PAGE_DATA.vadderaCulture.en;

  return constructMetadata({
    title: data.title,
    description: data.description,
    path: "/vaddera-culture",
    locale,
    keywords: data.keywords,
    image: "/vaddera-culture-work.jpg",
    type: "article",
  });
}

export default async function VadderaCulturePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations({ locale, namespace: "vadderaCulture" });
  const isTelugu = locale === "te";
  ...
  return (
    <div className="bg-amber-50 text-gray-900 flex flex-col items-center justify-center overflow-hidden">
      ...
      <ArticleJsonLd
        title={isTelugu ? SEO_PAGE_DATA.vadderaCulture.te.title : SEO_PAGE_DATA.vadderaCulture.en.title}
        description={isTelugu ? SEO_PAGE_DATA.vadderaCulture.te.description : SEO_PAGE_DATA.vadderaCulture.en.description}
        url={`/${locale}/vaddera-culture`}
        image="/vaddera-culture-work.jpg"
      />
      <BreadcrumbJsonLd
        items={[
          { name: isTelugu ? "హోమ్" : "Home", url: `/${locale}` },
          { name: isTelugu ? "సంస్కృతి" : "Culture", url: `/${locale}/vaddera-culture` },
        ]}
      />

      {/* ======= Hero Section ======= */}
      <header className="w-full max-w-6xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-8 px-4 sm:px-6 md:px-10 py-12 md:py-20">
        {/* Left Side Text */}
        <div className="flex-1 text-center lg:text-left">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-red-800 mb-6 drop-shadow-sm">
            {t("hero.title")}
          </h1>
        ...
        <div className="flex-1 flex justify-center lg:justify-end">
          <Image
            ...
            alt="Historic stone sculpting and cultural craftsmanship of Vaddera artisan community"
            width={1040}
            height={527}
            priority
